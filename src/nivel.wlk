...
object nivel{
	...
	//	CONFIGURACI�N DEL JUEGO
		...
		game.whenCollideDo(espatula, { w =>  if (bobVisual.equals(w)) espatula.serTomada(bobVisual) })
		game.whenCollideDo(plankton, { w =>  if (bobVisual.equals(w)) plankton.serTomada(bobVisual) })
		
		ingredientesSueltos.forEach { it => game.whenCollideDo(it, { g =>	
			if (bobVisual.equals(g)) 
