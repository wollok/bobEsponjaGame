...
class MarcoSolido {
	...
	method dibujarLineaVertical(columna, filaDesde, filaHasta, direccion) {
		(filaDesde .. filaHasta).forEach(
			{ fila => self.dibujarElemento(columna, fila, direccion) }
		)
	}
	
	method dibujarLineaHorizontal(fila, columnaDesde, columnaHasta, direccion) {
		(columnaDesde .. columnaHasta).forEach(
			{ columna => self.dibujarElemento(columna, fila, direccion) }
		)
	}
}
